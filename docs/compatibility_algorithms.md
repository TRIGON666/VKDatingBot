...
avg_diff_group = средняя разница по чертам внутри каждой группы
weighted_avg_diff = сумма(avg_diff_group * weight_group) / сумма_доступных_весов
psychology_score = 100 - weighted_avg_diff
```

Если по какой-то группе нет данных, ее вес не учитывается.
...
## HTML-отчет для администратора

Для диплома можно описать HTML-отчет за месяц как отдельный модуль аналитики.
...
В HTML-отчете это можно показать:
...
10. Проверить, что жалоба появляется в HTML-отчете администратора.

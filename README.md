...
- `/admin_stats` или `/admin_nlp` - несколько PNG-страниц с карточками KPI, столбчатыми и круговыми диаграммами.
- `/admin_month_file` или `/admin_report_file` - скачиваемый ZIP с HTML-отчетом за месяц, круговыми диаграммами, фидбеками и процентами совместимости.
- `/admin_stats_check` или `/admin_check_stats` - проверка, что сбор статистики, PNG-отчет и ZIP-отчет собираются без ошибок.
- `/admin_seed_stats` или `/admin_fake_stats` - создать искусственную демо-статистику для проверки графиков и отчетов.

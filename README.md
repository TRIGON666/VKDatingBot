...
- `/admin_month_file` или `/admin_report_file` - скачиваемый ZIP с HTML-отчетом за месяц: KPI, топ событий, жалобы, NLP-метрики, фидбеки и проценты совместимости.
- `/admin_stats_check` или `/admin_check_stats` - техническая проверка, что данные и HTML/ZIP-отчет собираются без ошибок.
- `/admin_seed_stats` или `/admin_fake_stats` - создать искусственную демо-статистику для проверки HTML-отчета.

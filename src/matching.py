...
DEFAULT_NLP_MODEL_PATH = str(
    Path(__file__).resolve().parents[1] / "models" / "nlp_model.pkl"
)
LEGACY_NLP_MODEL_PATH = str(
    Path(__file__).resolve().parents[1] / "data" / "models" / "text_compatibility.joblib"
)


def _resolve_model_path() -> str:
    """Возвращает путь к модели с приоритетом env -> новый путь -> legacy."""
    env_path = os.getenv("NLP_MODEL_PATH", "").strip()
    if env_path:
        return env_path
    if os.path.exists(DEFAULT_NLP_MODEL_PATH):
        return DEFAULT_NLP_MODEL_PATH
    return LEGACY_NLP_MODEL_PATH
...
def rank_matches(
    ...
    for cand in candidates:
        ...
        # Формула текста: 0.4 * TF-IDF + 0.6 * NLP.
        if nlp_score is None:
            text_score = tfidf_score
        else:
            text_score = round((tfidf_score * 0.4) + (nlp_score * 0.6), 2)

...
class NLPModelManager:
    ...
    def _list_backup_names(self) -> list[str]:
        if not os.path.exists(self.backup_dir):
            return []
        return sorted([f for f in os.listdir(self.backup_dir) if f.endswith(".pkl")], reverse=True)
    ...
    def status(self):
        ...
        print("\nФайлы данных:")
        data_files = {
            "Данные обучения": "data/nlp_training_data.csv",
            "Метрики": "data/nlp_metrics.jsonl",
            "Отчет": "reports/nlp_latest_report.json",
        }
    ...
    def backup(self):
        ...
        backup_path = os.path.join(self.backup_dir, f"nlp_model_{timestamp}.pkl")
...
def main():
    ...
    parser.add_argument("--model-path", default="models/nlp_model.pkl")
